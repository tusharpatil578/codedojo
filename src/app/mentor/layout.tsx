...
import { Home, Users, Calendar, HelpCircle, LogOut, User, Menu, X } from 'lucide-react';
...
export default function MentorLayout({ children }: { children: React.ReactNode }) {
  ...
  const [user, setUser] = useState<MentorSession | null>(null);
  ...
  useEffect(() => {
    async function fetchUser() {
      try {
        ...
        if (data.authenticated && data.user.role === 'MENTOR') {
          setUser(data.user);
        } else {
          router.push('/login');
        }
      ...
      <aside className="w-64 bg-[#0D1321] border-r border-white/5 flex flex-col justify-between hidden md:flex shrink-0">
        <div className="p-6 space-y-6">
          <Link href="/" className="text-lg font-extrabold tracking-wider text-white">
            CODE<span className="text-[#808000]">DOJO</span> <span className="text-[10px] font-bold bg-[#808000]/15 text-[#808000] px-1.5 py-0.5 rounded ml-1">MENTOR</span>
          </Link>
          ...
          <nav className="space-y-1.5 pt-4">
            <Link 
              ...
              className={`flex items-center gap-2.5 px-4 py-2.5 rounded-lg text-xs font-bold transition-all ${
                pathname === '/mentor/dashboard' ? 'bg-[#808000] text-white' : 'text-gray-400 hover:bg-white/5 hover:text-white'
              }`}
            >
              <Home size={16} /> Mentor Dashboard
            </Link>
          </nav>
        </div>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="h-16 border-b border-white/5 bg-[#0D1321]/40 flex items-center justify-between px-6 md:justify-end">
          ...
          <div className="flex items-center gap-2 text-xs font-semibold">
            ...
            <span className="text-gray-400">Mentor Portal Live</span>
          </div>
        </header>
